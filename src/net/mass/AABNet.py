...
class AABNet(nn.Module):
    ...
    def __init__(self, nInputChannels=3, n_classes=1, os=16, pretrained=False, freeze_bn=False, _print=True):
        ...
        self.decoder1 = DecoderBlock(1280, 512)
        ...
        self.last_conv = nn.Sequential(nn.Conv2d(432, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm2d(256),
                                       nn.ReLU(),
    ...
    def forward(self, input):
        ...
        x = self.encoder2(x)
        # print("encoder2", x.shape)  # 2, 128, 128, 128
        low_2 = x
        x = self.encoder3(x)
        x = self.encoder4(x)
        ...
        low_2 = self.arem(low_2)
        low_2 = F.interpolate(low_2, size=low_level_features.size()[2:], mode='bilinear', align_corners=True)
        # 拼接低层次的特征，然后再通过插值获取原图大小的结果
        x = torch.cat((d1, low_level_features, low_2), dim=1)
        # print("xzzzz", x.shape) #2, 432, 256, 256
        x = self.last_conv(x)
        # 实现插值和上采样
