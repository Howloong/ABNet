...
class AADEFNet(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=1, os=16, pretrained=False, freeze_bn=False, _print=True):
        ...
        self.last_3_conv = nn.Sequential(nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm2d(16),
                                       nn.ReLU(),
    ...
    def forward(self, input):
        ...
        # print("before:", x.shape)   #4, 304, 256, 256
        x = self.last_1_conv(x)
        # print("after_1:", x.shape)
        x = self.last_2_conv(x)
        # print("after_2:", x.shape)
        x = self.last_3_conv(x)
        # print("after_3:", x.shape)  #4, 1, 256, 256
        # 实现插值和上采样
